...
public final class LongArrayEventHandler_Object  extends BaseEventHandler<LongArray_Object>//implements EventHandler<LongArray_Object>, BatchStartAware
{
    ...
    @Override
    public void onEvent(final LongArray_Object event, final long sequence, final boolean endOfBatch) throws Exception
    {
        //for (int i = 0; i < event.ll.length; i++)
        ...
        //for (int i = 0; i < event.ll.length; i++)
        {
            ...
            //value.set(value.get() + event.ll[i]);
            //value.set(value.get() + 1);

            //event.ll.setCharAt(i,'q');
            event.transform();
